...
const ConsultationBooker: React.FC<ConsultationBookerProps> = ({
  onBack,
  onComplete,
  serviceName = 'project',
  ...
  const consultationOptions: ConsultationOption[] = [
    {
      id: 'call',
      title: 'Phone Call',
      description: 'Quick chat to discuss your requirements',
      icon: Phone,
      duration: '15-30 mins',
      available: true,
    },
    {
      id: 'video',
      title: 'Video Call',
      description: 'Screen share your ideas and inspiration',
      icon: Video,
      duration: '30 mins',
      available: true,
    },
    {
      ...
      description: 'See samples and meet the team in Kidderminster',
      icon: MapPin,
      duration: '45 mins',
      available: true,
    },
      ...
      {!showCalendar ? (
        <>
          ...
          >
            ...
            >
              <Calendar className="w-8 h-8" style={{ color: printingColors.accent }} />
            </div>
            ...
            <p className="neuzeit-light-font text-white/60 max-w-md mx-auto">
              Chat with our team about your {serviceName}. No obligation, just friendly expert advice.
            </p>
          </motion.div>
            ...
            {consultationOptions.map((option, index) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: `${printingColors.accent}25` }}
                >
                  <option.icon className="w-7 h-7" style={{ color: printingColors.accent }} />
                </div>
            ...
            {selectedType && (
              <>
                <div
                  ...
                  style={{ backgroundColor: `${printingColors.accent}25` }}
                >
                  {selectedType === 'call' && <Phone className="w-6 h-6" style={{ color: printingColors.accent }} />}
                  {selectedType === 'video' && <Video className="w-6 h-6" style={{ color: printingColors.accent }} />}
                  {selectedType === 'inperson' && <MapPin className="w-6 h-6" style={{ color: printingColors.accent }} />}
                </div>
                <div>
          ...
          <div className="mt-8 p-5 rounded-[15px] bg-white/5 border border-white/10">
            ...
            <ul className="space-y-2">
              {[
                ...
                'Get expert advice on materials and options',
                'Receive a detailed quote (no obligation)',
                'Ask any questions you have',
              ].map((item, i) => (
                <li key={i} className="flex items-center gap-3">
                  <CheckCircle className="w-4 h-4 flex-shrink-0" style={{ color: printingColors.accent }} />
