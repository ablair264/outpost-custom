...
export function ProductTypesView() {
  ...
  const fetchProductTypes = useCallback(
    async (nextCursor?: string | null, reset = false) => {
      try {
        ...
        const response = await fetch(`${API_BASE}/products-admin/product-types?${params}`, {
  ...
  const handleRowClick = (productType: string) => {
    navigateTo(
      'styles',
      { brand: activeBrand || undefined, productType },
      productType
    );
  };

  const columns: ColumnDef<ProductTypeRow, any>[] = [
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        <button
          onClick={() => fetchProductTypes(null, true)}
    ...
    <div className="h-full overflow-auto">
      ...
      <div ref={loadMoreRef} className="h-20 flex items-center justify-center">
        ...
        {!hasMore && data.length > 0 && (
          <span className="text-gray-500 text-sm">All {data.length} product types loaded</span>
        )}
      </div>
    </div>
  );
